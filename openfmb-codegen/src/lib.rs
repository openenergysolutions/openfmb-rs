...
//! ```rust,no_run
//! fn main() {
//!     openfmb_codegen::compile_protos(&["src/items.proto"],
//!                                 &["src/"]).unwrap();
//! }
//! ```
//!
...
impl Config {
    ...
    ///
    /// ```rust
    /// # let mut config = openfmb_codegen::Config::new();
    ...
    ///
    /// This method is like the `openfmb_codegen::compile_protos` function, with the added ability to
    ...
    /// ```rust,no_run
    /// fn main() {
    ///     let mut openfmb_codegen = openfmb_codegen::Config::new();
    ///     openfmb_codegen.btree_map(&["."]);
    ///     openfmb_codegen.compile_protos(&["src/frontend.proto", "src/backend.proto"],
    ///                                &["src"]).unwrap();
    /// }
    /// ```
