...
use proto_types::field_descriptor_proto::{Label, Type};
use proto_types::source_code_info::Location;
use proto_types::{
...
impl<'a> CodeGenerator<'a> {
    ...
    fn append_message(&mut self, message: DescriptorProto) {
        ...
        for (field, idx) in &fields {
                ...
                .and_then(|type_name| map_types.get(type_name))
            {
                Some(&(ref key, ref value)) => {
                    self.append_map_field(&fq_message_name, field.clone(), key, value)
                }
                None => self.append_field(&fq_message_name, field.clone()),
            }
            self.path.pop();
        }
        self.path.pop();
        ...
        // generate static defaults with lazy init
        self.push_indent();
        ...
        self.depth += 1;
        self.push_indent();
        self.buf.push_str("use lazy_static::lazy_static;\n");
        self.push_indent();
        self.buf.push_str("lazy_static! {\n");
        self.depth += 1;
        for (field, _idx) in &fields {
            let ty = self.absolute_type(&field);
            let optional = self.optional(&field);
            let repeated = field.label == Some(Label::Repeated as i32);
            if optional {
                self.push_indent();
                self.buf.push_str("pub(super) static ref ");
    ...
    fn append_field(&mut self, msg_name: &str, field: FieldDescriptorProto) {
        ...
        if let Some(ref field_options) = field.options {
            debug!(
                "    parent_message: {:?}, required_field: {:?}, multi_min: {:?}, multi_max: {:?}, uuid: {:?}, key: {:?}",
                field_options.parent_message,
                field_options.required_field,
    ...
    /// Get the absolute path to the type (::xyz::Abc)
    fn absolute_type(&self, field: &FieldDescriptorProto) -> String {
        match field.r#type() {
            Type::Float => String::from("f32"),
            Type::Double => String::from("f64"),
            Type::Uint32 | Type::Fixed32 => String::from("u32"),
            ...
            Type::String => String::from("std::string::String"),
            Type::Bytes => String::from("std::vec::Vec<u8>"),
            Type::Group | Type::Message => self.absolute_ident(field.type_name()),
        }
    }

    fn absolute_ident(&self, pb_ident: &str) -> String {
        // protoc should always give fully qualified identifiers.
        assert_eq!(".", &pb_ident[..1]);
        ...
        let local_path = self.package.split('.').peekable();
